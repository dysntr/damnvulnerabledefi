const exchangeJson = require("../contracts/build-uniswap-v1/UniswapV1Exchange.json")
const factoryJson = require("../contracts/build-uniswap-v1/UniswapV1Factory.json")
...
describe("[Challenge] Puppet", function () {
    ...
    it("Exploit", async function () {
        ...
        console.log(
            "\t Before Attack: lendingPool's Token balance:",
            (await token.balanceOf(lendingPool.address)).toString()
        )
        console.log(
            "\t Before Attack: Attacker's Token balance:",
            (await token.balanceOf(attacker.address)).toString()
        )

        console.log(
            "\t Before Attack: lendingPool.calculateDepositRequired(100000):",
            (await lendingPool.calculateDepositRequired(100000)).toString()
        )

        //trade all your tokens for eth
        ...
        console.log(
            ...
            (await token.balanceOf(this.lendingPool.address)).toString()
        )

        console.log(
            "\t After Attack: Attacker's Token balance:",
            (await this.token.balanceOf(attacker.address)).toString()
        )

        console.log(
            "\t After Attack: Attacker's Eth balance:",
